...
export function renderWhy() {
  return `<div>
    ...
    <h1 style="font-family:'Space Grotesk',sans-serif;font-weight:600;font-size:clamp(32px,4.5vw,46px);line-height:1.05;letter-spacing:-.03em;margin:0 0 18px;text-wrap:balance;">Why the current model fails.</h1>
    ...
    <div class="thread-rail">
      ...
      <section class="thread-section" id="status-quo">
        ...
        <h2 class="h2">Four tools and no source of truth.</h2>
      ...
      <section class="thread-section" id="stale">
        ...
        <p class="body">The staleness is built into the architecture. A hand-maintained artifact can only reflect what its author knew at save time, and nothing keeps it current because the data it summarizes lives somewhere else entirely.</p>
        <div class="card" style="border-left:3px solid var(--bad);">
          <div class="card-title">The cost</div>
          <div class="card-body" style="margin-top:4px;">Decisions are made on stale data, leaders steer by the deck while the portfolio drifts elsewhere, and the gap grows until a failure surfaces on its own.</div>
        </div>
      </section>
      ...
      <section class="thread-section" id="rag">
        ...
        <div class="card" style="border-left:3px solid var(--warn);">
          ...
          <div class="card-body" style="margin-top:4px;">Status is computed from the documents themselves: whether every requirement traces to a test, which risks are open and who owns them, whether the approved documents still pass their audits, so the checks pick the color instead of a person.</div>
        </div>
      </section>
      ...
      <section class="thread-section" id="governance">
        ...
        <p class="body">A stage gate amounts to an email in which someone forwards a deck and someone else replies "approved." Three months later, no one can find the thread, no one agrees on what was actually approved, and there's no record of what conditions the approval was contingent on.</p>
        ...
        <div class="card" style="border-left:3px solid var(--ok);">
          ...
          <div class="card-body" style="margin-top:4px;">A gate becomes a CI check that runs against defined criteria and records its result in the repository, and the approval becomes a merged pull request with a reviewer on record. The audit trail accumulates as a by-product of working.</div>
        </div>
      </section>
    ...
    <div style="margin-top:48px;padding:22px 24px;background:var(--accent-soft);border:1px solid var(--accent-line);border-radius:13px;">
      ...
      <p style="font-size:16px;color:var(--ink);margin:0 0 16px;max-width:60ch;">The Manifesto names the five value-pairs behind the response, and the Rosetta Stone maps each problem to its as-code equivalent. The case study shows the whole approach against a real document set.</p>
